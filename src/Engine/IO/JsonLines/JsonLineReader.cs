...
public sealed class JsonLineReader : IDisposable
{
    ...
    )
    {
        ...
        while (linesRead < linesToRead)
        {
            ...
            // Remove trailing newline characters for validation
            var validationSpan = lineBytes.AsSpan();
            var trimmedSpan = TrimNewline(validationSpan);
    ...
    private static void ValidateJsonLine(ReadOnlySpan<byte> lineBytes)
    {
        ...
        catch (JsonException ex)
        {
            throw new InvalidDataException(
                $"Invalid JSON line at byte position {reader.BytesConsumed}",
                ex
            );
        }
    }
